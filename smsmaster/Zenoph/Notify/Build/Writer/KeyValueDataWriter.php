 ...
 */
    ...
    abstract class KeyValueDataWriter extends DataWriter {
     ...
     /*   
        protected function writeAuthData(&$dataArr) {
            // perform validation
            $this->assertAuthData();
            
            switch ($this->_authModel){
                case AuthModel::PORTAL_PASS:
                    $this->appendKeyValueData($dataArr, "model", "ppass");
                    $this->appendKeyValueData($dataArr, "factor", $this->generateAuthFactor());
                    break;
                    
                case AuthModel::API_KEY:
                    $this->appendKeyValueData($dataArr, "model", "apikey");
                    $this->appendKeyValueData($dataArr, "apiKey", $this->_authApiKey);
                    break;
            ...
            // general data
        //    $this->appendKeyValueData($dataArr, "pid", $this->_authPID);
            
            if ($this->_authLoadAPS)
                $this->appendKeyValueData ($dataArr, "ldas", "true");
        }
        
        public function &writeAuthRequest() {
            // only authentication data
            $this->writeAuthData($this->_keyValueArr);
      ...
      */  
        public function &writeSMSRequest($tmcArr) {
            if (is_null($tmcArr) || !is_array($tmcArr) || count($tmcArr) == 0)
                throw new \Exception('Invalid reference for writing text message request.');
        ...
        private function writeSMSProperties($tmc, &$store){
            if (is_null($tmc) || $tmc instanceof SMSComposer === false)
                throw new \Exception('Invalid object reference for writing text message properties.');
            ...
            if ($tmc->getMessageVariablesCount($messageText) > 0){
        ...
        protected function writeCommonMessageProperties($mc, &$store) {
            if (is_null($mc) || $mc instanceof VoiceComposer)
        ...
        protected function writeDestinations($mc, &$store) {
            ...
            foreach ($compDestsList as $compDest){
                ...
                // if message notifications to like SMS and Voice
            ...
            // append destinations
            $this->appendKeyValueData($store, "destinations", $destsStr);
        ...
        public function &writeDestinationsDeliveryRequest($templateId, $messageIdsArr) {
            if (is_null($templateId) || empty($templateId))
                throw new \Exception('Invalid message template identifier for writing destinations delivery request.');
        ...
        //    $this->writeAuthData($store);
            ...
            // append message Ids
            $this->appendKeyValueData($store, "destinations", $idsStr);
        ...
        private function writeScheduledMessageDestinations(MessageComposer $mc, &$store){
            ...
            if (!empty($addDestStr)) {
                $this->appendKeyValueData($store, "add-destinations", $addDestStr);
                
                if (!empty($addValuesStr))
                    $this->appendKeyValueData($store, "add-values", $addValuesStr);
            }
            
            if (!empty($updateDestStr)){
                $this->appendKeyValueData($store, "upd-destinations", $updateDestStr);
                
                if (!empty($updateValuesStr))
                    $this->appendKeyValueData ($store, "upd-values", $updateValuesStr);
            }
            
            if (!empty($deleteDestStr))
                $this->appendKeyValueData($store, "del-destinations", $deleteDestStr);
        }
        ...
        public function &writeCancelScheduleRequest($templateId) {
            if (is_null($templateId) || empty($templateId))
                throw new \Exception('Invalid message template identifier for cancelling message scheduling.');
